...
pub fn workspace_wr() {
    ...
    match cell_1_r_res {
        TxnExecResult::Accepted(cell) => {
            ...
            assert_eq!(cell.data.Map().unwrap().get_static_key("id").unwrap().I64().unwrap(), 100);
            assert_eq!(cell.data.Map().unwrap().get_static_key("name").unwrap().String().unwrap(), "Jack");
            assert_eq!(cell.data.Map().unwrap().get_static_key("score").unwrap().U64().unwrap(), 70);
        },
    ...
    match cell_1_r_res {
        TxnExecResult::Accepted(cell) => {
            assert_eq!(cell.id(), cell_1.id());
            assert_eq!(cell.data.Map().unwrap().get_static_key("score").unwrap().U64().unwrap(), 90);
        },
        _ => {panic!("read cell 1 not accepted {:?}", cell_1_r_res)}
    }
...
pub fn data_site_wr() {
    ...
    assert_eq!(cell_r2.data.Map().unwrap().get_static_key("id").unwrap().I64().unwrap(), 100);
    assert_eq!(cell_r2.data.Map().unwrap().get_static_key("name").unwrap().String().unwrap(), "Jack");
    assert_eq!(cell_r2.data.Map().unwrap().get_static_key("score").unwrap().U64().unwrap(), 90);
}

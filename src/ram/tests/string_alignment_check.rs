...
fn check_string_alignment_value() {
    ...
    for offset in test_offsets {
        ...
        println!("  offset {} → aligned {} (adjustment: {})", offset, aligned, adjustment);
    }
    ...
    if string_align == 4 {
        ...
        println!("❌ String alignment is {} bytes (unexpected!)", string_align);
    }
}
...
fn test_alignment_math_for_static_bound() {
    ...
    if old_aligned == new_aligned {
        println!("✓ Both schemas align first string to same offset: {}", old_aligned);

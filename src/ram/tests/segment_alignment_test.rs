...
fn test_segment_initial_alignment() {
    ...
    for i in 0..10 {
        let seg = allocator.alloc_seg(&None, &None).expect("Failed to allocate segment");
        println!("Segment {} address: 0x{:016x} (8-byte aligned: {})", 
        ...
        assert_eq!(seg.append_header.load(std::sync::atomic::Ordering::Acquire) % 8, 0,
...
fn test_segment_try_acquire_alignment() {
    ...
    for size in sizes {
        ...
        let addr = seg.try_acquire(size).expect(&format!("Failed to acquire {} bytes", size));
        println!("Acquired {} bytes at 0x{:016x} (8-byte aligned: {})", 
                 size, addr, addr % 8 == 0);
        assert_eq!(addr % 8, 0, "Acquired address must be 8-byte aligned (size={})", size);
    }
    println!();
...
fn test_segment_concurrent_allocation_alignment() {
            ...
            thread::spawn(move || {
                ...
                for i in 0..allocations_per_thread {
                    if let Some(seg) = allocator.alloc_seg(&None, &None) {
                        if seg.addr % 8 != 0 {
                            eprintln!("❌ Thread {} allocation {} got misaligned address: 0x{:016x}",
    ...
    for (thread_id, handle) in handles.into_iter().enumerate() {
        ...
        if misaligned > 0 {
            println!("❌ Thread {} had {} misaligned allocations", thread_id, misaligned);
        }
        total_misaligned += misaligned;
    }
    
    println!();
    println!("Total allocations: {}", num_threads * allocations_per_thread);
    println!("Misaligned allocations: {}", total_misaligned);
    
    assert_eq!(total_misaligned, 0, "Found {} misaligned segment allocations", total_misaligned);
...
fn test_segment_try_acquire_concurrent_alignment() {
    ...
    let allocator = SegmentAllocator::new(0, SEGMENT_SIZE * 10);
    let seg = Arc::new(allocator.alloc_seg(&None, &None).expect("Failed to allocate segment"));
            ...
            thread::spawn(move || {
                ...
                for i in 0..tries_per_thread {
                    ...
                    if let Some(addr) = seg.try_acquire(size) {
                        if addr % 8 != 0 {
                            eprintln!("❌ Thread {} try {} got misaligned address: 0x{:016x} (size={})",
    ...
    for (thread_id, handle) in handles.into_iter().enumerate() {
        ...
        println!("Thread {} acquired {} addresses ({} misaligned)", 
    ...
    assert_eq!(total_misaligned, 0, "Found {} misaligned addresses from try_acquire", total_misaligned);
...
fn test_segment_append_header_increments() {
    ...
    for (size, description) in sizes {
        ...
        if let Some(addr) = seg.try_acquire(size) {
            ...
            println!("{}", description);
            println!("  Before:   0x{:016x} (offset: {})", addr_before, addr_before % 8);
            println!("  Acquired: 0x{:016x} (offset: {})", addr, addr % 8);
            println!("  After:    0x{:016x} (offset: {})", addr_after, addr_after % 8);
            println!("  Increment: {} bytes", addr_after - addr_before);
            
            assert_eq!(addr, addr_before, "Acquired address should match pre-increment header");
            assert_eq!(addr % 8, 0, "Acquired address must be 8-byte aligned");
            assert_eq!(addr_after % 8, 0, "After increment, header must remain 8-byte aligned");
            assert_eq!(addr_after - addr_before, size as usize, "Increment should match requested size");
            println!("  ✓");
            println!();
        }
    }
...
fn test_detect_address_corruption_pattern() {
    ...
    for addr in corrupted_addresses {
        ...
        println!("  2-byte alignment: offset={} {}", offset_2, if offset_2 == 0 { "✓" } else { "✗" });
        println!("  4-byte alignment: offset={} {}", offset_4, if offset_4 == 0 { "✓" } else { "✗" });
        println!("  8-byte alignment: offset={} {}", offset_8, if offset_8 == 0 { "✓" } else { "✗" });
        ...
        println!("  Should be (round down): 0x{:016x}", should_be_rounded_down);

...
use crate::ram::io::{align_address_with_ty, align_address};
...
fn test_alignment_propagation_from_base() {
    ...
    println!("Base address:     0x{:016x} (aligned: {})", correct_base, correct_base % 8 == 0);
    
    let addr_0 = correct_base + field_offset_0;
    let addr_8 = correct_base + field_offset_8;
    
    println!("Field at offset 0:  0x{:016x} (4-byte aligned: {})", addr_0, addr_0 % 4 == 0);
    println!("Field at offset 8:  0x{:016x} (4-byte aligned: {})", addr_8, addr_8 % 4 == 0);
    ...
    println!("Base address:     0x{:016x} (aligned: {}, offset: {})", 
             corrupted_base, corrupted_base % 8 == 0, corrupted_base % 8);
    ...
    println!("Field at offset 0:  0x{:016x} (4-byte aligned: {}, actual offset: {})", 
             addr_0_bad, addr_0_bad % 4 == 0, addr_0_bad % 4);
    println!("Field at offset 8:  0x{:016x} (4-byte aligned: {}, actual offset: {})", 
             addr_8_bad, addr_8_bad % 4 == 0, addr_8_bad % 4);
    ...
    println!("Misaligned base:  0x{:016x} (offset: {})", misaligned_base, misaligned_base % 8);
    
    // Try to align an offset from this base
    let field_offset = 0;
    let aligned_offset = align_address_with_ty(Type::U32, field_offset);
    println!("Aligned offset:   {} (this is correct - offset 0 is already aligned)", aligned_offset);
    
    let final_address = misaligned_base + aligned_offset;
    println!("Final address:    0x{:016x} (4-byte aligned: {})", 
    ...
    println!("Correct final addr:  0x{:016x} (4-byte aligned: {})", 
             correct_final, correct_final % 4 == 0);
    ...
    assert_eq!(corrupted_base % 8, 6, "Crash address has offset 6 from 8-byte boundary");
    assert_eq!(corrupted_base % 4, 2, "Crash address has offset 2 from 4-byte boundary");
...
fn test_cell_location_must_be_8_byte_aligned() {
    ...
    for base in &invalid_bases {
        let offset = base % 8;
        println!("  0x{:016x} - offset: {}, 4-byte aligned: {}", 
                 base, offset, base % 4 == 0);
        ...
        if u32_addr % 4 != 0 {
            println!("    ❌ Reading u32 at 0x{:016x} would PANIC (not 4-byte aligned)", u32_addr);
        }
    }
    println!();
...
fn test_alignment_math() {
    ...
    for alignment in [2, 4, 8, 16] {
        ...
        println!("{}-byte alignment: offset={}, aligned={}", alignment, offset, is_aligned);
        
        if !is_aligned {
            let next_aligned = addr + (alignment - offset);
            println!("  Next {}-byte aligned address: 0x{:016x}", alignment, next_aligned);
        }
    }
    println!();

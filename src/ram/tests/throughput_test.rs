...
fn test_write_throughput_with_wal_and_backup() {
    ...
    println!(
        "\n=== Writing {:.2} GB of data ({} cells) ===",
        target_gb, total_writes
    );
    ...
    for thread_id in 0..NUM_THREADS {
        ...
        let handle = thread::spawn(move || {
            ...
            for i in start_id..end_id {
                ...
                data_map.insert(
                    &String::from("name"),
                    OwnedValue::String("x".repeat(cell_data_size)),
                );
    ...
    if throughput_mbs < 20.0 {
        println!(
            "⚠️  WARNING: Throughput is low ({:.2} MB/s)",
            throughput_mbs
        );
        ...
        println!(
            "✓ Good: Throughput is reasonable ({:.2} MB/s)",
            throughput_mbs
        );
        println!(
            "  Group commit batching is working with {} threads.",
            NUM_THREADS
        );
    } else {
    ...
    assert!(
        ...
        total_writes,
        cell_count,
        cell_count as i64 - total_writes as i64
    );

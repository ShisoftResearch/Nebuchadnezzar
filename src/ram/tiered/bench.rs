...
impl ZipfGenerator {
    ...
    fn sample(&self, rng: &mut impl Rng) -> usize {
        ...
        match self.sum_probs.binary_search_by(|p| p.partial_cmp(&u).unwrap()) {
            Ok(i) => i,
...
impl BenchResult {
    ...
    fn print(&self) {
        ...
        info!("╠══════════════════════════════════════════════════════════════╣");
        info!("║ Total Operations:    {:>10}                             ║", self.total_ops);
        info!("║ Total Duration:      {:>10.2} s                           ║", self.duration.as_secs_f64());
        info!("║ Throughput:          {:>10.2} ops/s                       ║", self.throughput_ops);
        info!("╠══════════════════════════════════════════════════════════════╣");
        info!("║ Latency Statistics:                                          ║");
        info!("║   Average:           {:>10.2} μs                          ║", self.avg_latency_us);
        info!("║   p50 (median):      {:>10.2} μs                          ║", self.p50_latency_us);
        info!("║   p95:               {:>10.2} μs                          ║", self.p95_latency_us);
        info!("║   p99:               {:>10.2} μs                          ║", self.p99_latency_us);
        info!("╚══════════════════════════════════════════════════════════════╝");
        info!("");
    }
}
...
fn bench_hot_segment_reads() {
    ...
    for i in 0..num_cells {
        ...
        data_map.insert(&String::from("name"), OwnedValue::String(format!("hot_{}", i)));
...
fn bench_cold_segment_reads() {
    ...
    std::env::set_var("NEB_TIERED_PHYSICAL_MEMORY_LIMIT", &format!("{}", 1 * SEGMENT_SIZE));
    ...
    for i in 0..num_cells {
        ...
        data_map.insert(&String::from("name"), OwnedValue::String(format!("cold_{}", i)));
        data_map.insert(&String::from("data"), OwnedValue::String(large_data.clone()));
...
fn bench_mixed_uniform() {
    ...
    for i in 0..num_cells {
        ...
        data_map.insert(&String::from("name"), OwnedValue::String(format!("mixed_{}", i)));
        data_map.insert(&String::from("data"), OwnedValue::String(large_data.clone()));
    ...
    let hot_count = chunks.list[0].segments().iter().filter(|s| s.is_hot()).count();
    let cold_count = chunks.list[0].segments().iter().filter(|s| s.is_cold()).count();
    info!("Created {} hot segments, {} cold segments", hot_count, cold_count);
    ...
    let final_hot = chunks.list[0].segments().iter().filter(|s| s.is_hot()).count();
    let final_cold = chunks.list[0].segments().iter().filter(|s| s.is_cold()).count();
    info!("After benchmark: {} hot segments, {} cold segments", final_hot, final_cold);
...
fn bench_mixed_zipf() {
    ...
    for i in 0..num_cells {
        ...
        data_map.insert(&String::from("name"), OwnedValue::String(format!("zipf_{}", i)));
        data_map.insert(&String::from("data"), OwnedValue::String(large_data.clone()));

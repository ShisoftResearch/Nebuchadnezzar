...
impl NebServer {
    ...
    pub async fn shutdown(&self) {
        ...
        if let Ok(lsm_client) = self.get_lsm_tree_service().await {
            ...
            // What this session could not index, said once where someone is
            // looking. Both are debts the next start pays off by reconciling
            // against the cells; neither is silent.
            let abandoned = crate::index::ranged::tree::btree::storage::INDEX_PAGES_ABANDONED
                .load(std::sync::atomic::Ordering::Relaxed);
            if abandoned > 0 {
                error!(
                    "{} index page image(s) were abandoned this session -- entries they carried \
                     never reached disk, with no index task reporting failure. The next start \

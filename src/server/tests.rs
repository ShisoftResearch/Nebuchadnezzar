...
pub async fn schema_wal_recovery_test() {
    ...
    info!("PHASE 1: Creating server and adding 3 schemas (below snapshot threshold)");
    {
        ...
        client.new_schema_with_id(schema1.clone()).await.unwrap().unwrap();
        client.new_schema_with_id(schema2.clone()).await.unwrap().unwrap();
        client.new_schema_with_id(schema3.clone()).await.unwrap().unwrap();
        
        info!("Created 3 schemas, waiting for WAL commit...");
        tokio::time::sleep(tokio::time::Duration::from_secs(3)).await;
        
        // Verify schemas exist before shutdown
        let retrieved = client.schema_by_name(&"users".to_string()).await.unwrap().unwrap();
    ...
    info!("PHASE 2: Restarting server - should replay WAL logs to rebuild schemas");
    {
        ...
        let s1 = client2.schema_by_name(&"users".to_string()).await.unwrap().unwrap();
        assert_eq!(s1.id, 100);
        info!("✓ Schema 'users' recovered from WAL");
        
        let s2 = client2.schema_by_name(&"products".to_string()).await.unwrap().unwrap();
        assert_eq!(s2.id, 200);
        info!("✓ Schema 'products' recovered from WAL");
        
        let s3 = client2.schema_by_name(&"orders".to_string()).await.unwrap().unwrap();
        assert_eq!(s3.id, 300);
...
pub async fn schema_snapshot_recovery_test() {
    ...
    info!("PHASE 1: Creating initial server and adding schemas");
    {
        ...
        // Verify schemas exist before shutdown
        let retrieved_schema1 = client.schema_by_name(&"users".to_string()).await.unwrap().unwrap();
        assert_eq!(retrieved_schema1.id, 100);
        assert_eq!(retrieved_schema1.name, "users");
        
        let retrieved_schema2 = client.schema_by_name(&"products".to_string()).await.unwrap().unwrap();
        assert_eq!(retrieved_schema2.id, 200);
        
        let retrieved_schema3 = client.schema_by_name(&"orders".to_string()).await.unwrap().unwrap();
    ...
    info!("PHASE 2: Restarting server and verifying schema recovery");
    {
        ...
        let recovered_schema1 = client2.schema_by_name(&"users".to_string()).await.unwrap().unwrap();
        assert_eq!(recovered_schema1.id, 100);
        assert_eq!(recovered_schema1.name, "users");
        info!("✓ Schema 'users' recovered successfully");
        
        let recovered_schema2 = client2.schema_by_name(&"products".to_string()).await.unwrap().unwrap();
        assert_eq!(recovered_schema2.id, 200);
        assert_eq!(recovered_schema2.name, "products");
        info!("✓ Schema 'products' recovered successfully");
        
        let recovered_schema3 = client2.schema_by_name(&"orders".to_string()).await.unwrap().unwrap();
        ...
        let schema4 = Schema::new(
            ...
            None,
            Field::new_schema(vec![
                Field::new_unindexed("stock", Type::U64),
            ]),
            false,
            false,
        );
        ...
        assert!(new_id > 300, "New schema should have ID > 300, got {}", new_id);
...
pub async fn schema_persistence_multiple_restarts() {
    ...
    info!("=== FINAL VERIFICATION ===");
    {
        ...
        for i in 1..=3 {
            let schema_name = format!("schema_cycle_{}", i);
            let schema = client.schema_by_name(&schema_name).await.unwrap();
            assert!(schema.is_some(), "Schema from cycle {} should exist after all restarts", i);

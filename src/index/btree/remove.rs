...
where
    F: Fn(&mut RebalancingNodes<KS, PS>),
    KS: Slice<EntryKey> + Debug + 'static,
    PS: Slice<NodeCellRef> + 'static,
{
    ...
    if level == 0 {
        ...
        parent_right_right_guard = if parent_parent_guard.len() > parent_parent_remove_pos + 1
            && (!parent_half_full || !parent_right_half_full)
        {
    ...
    parent_remove_result.rebalancing =
        parent_right_right_guard.map(|parent_right_right_guard_stripped| RebalancingNodes {
    ...
    match search {
        MutSearchResult::Internal(mut sub_node) => {
            ...
            if let Some(mut rebalancing) = node_remove_res.rebalancing {
                debug!(
                    "Need to rebalance sub nodes, key {:?}, level {}",
                    key, level
                );
                if rebalancing.left_guard.is_empty() && !rebalancing.right_guard.is_empty() {
                    debug!(
                        "Remove {:?} sub level left node is empty, level {}",
                        key, level
                    );
                    Some(with_innode_removing(
                        ...
                        |rebalancing| {
                            ...
                            if !rebalancing.left_guard.is_empty()
                                || (level == 0
                                    && read_unchecked::<KS, PS>(&(*tree.root.read())).first_key()
                                        != rebalancing.left_guard.first_key())
                            {
                                return;
                            }
                            ...
                            let right_right_ref =
                                right_node.right_ref_mut_no_empty().map(|r| r.clone());
                            ...
                            *right_node = NodeData::Empty(box EmptyNode {
                                left: Some(left_ref.clone()),
                                right: left_ref.clone(),
                            });
                            left_node
                                .left_ref_mut()
                                .map(|r| *r = left_left_ref.unwrap());
                            left_node
                                .right_ref_mut_no_empty()
                                .map(|r| *r = right_right_ref.unwrap());
                            rebalancing
                                .right_right_guard
                                .left_ref_mut()
                                .map(|r| *r = left_ref);
                            ...
                            debug!(
                                "Removed sub level empty node, living node len {}, have {:?}",
                                left_node.len(),
                                left_node.keys()
                            );
                            rebalancing.parent.innode_mut().ptrs.as_slice()
                                [rebalancing.parent_pos + 1] = sub_node.clone();
                        },
                    ))
                } else if rebalancing.right_guard.is_empty() {
                    debug!(
                        "Remove {:?} sub level right node is empty, level {}",
                        key, level
                    );
                    Some(with_innode_removing(
                        key,
                        ...
                        |rebalancing| {
                            ...
                            rebalancing
                                .right_right_guard
                                .left_ref_mut()
                                .map(|r| *r = owned_left_ref);
                            rebalancing.parent.remove(rebalancing.parent_pos + 1);
                        },
                    ))
                } else if rebalancing.left_guard.cannot_merge()
                    || rebalancing.right_guard.cannot_merge()
                {
                    ...
                    <= KS::slice_len()
                {
                    ...
                    debug!(
                        "Remove {:?} sub level need to be merged, level {}",
                        key, level
                    );
                    Some(with_innode_removing(
                        key,
                        ...
                        |rebalancing| {
                            ...
                            rebalancing.parent.innode_mut().merge_children(
                                left_pos,
                                right_pos,
                                left_node,
                                right_node,
                ...
                // None
                } else if rebalancing.parent.len() == 1
                    && rebalancing.left_guard.is_empty()
                    && rebalancing.right_guard.is_empty()
                {

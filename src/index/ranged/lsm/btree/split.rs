        ...
        PS: Slice<NodeCellRef> + 'static,
{
    ...
    match &*read_unchecked::<KS, PS>(node_ref) {
        &NodeData::External(_) => {
            ...
            let key_index = n.keys
                .as_slice_immute()[..n.len]
                .binary_search(mid_key)
                .expect("Pivot key must be exists for rain");
            debug_assert_eq!(&n.keys.as_slice_immute()[key_index], mid_key);
            n.len = key_index; // All others will be ignored
            debug_assert_ne!(n.len, 0); // Assert no empty node after cut

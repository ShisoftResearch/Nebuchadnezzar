...
impl RangedQueryClient {
    pub async fn seek<'a>(&'a self, key: &EntryKey, ordering: Ordering) -> Option<cursor::ClientCursor<'a>> {
        ...
        loop {
            ...
            let (tree_id, tree_client, lower, upper) = self.locate_key_server(key, ensure_updated).await;
            match tree_client.seek(tree_id, key.clone(), ordering, 128).await.unwrap() {
                OpResult::Successful((cursor, init_entry)) => {
                    let tree_boundary = match ordering {
                        Ordering::Forward => upper, Ordering::Backward => lower
                    };
                    if let Some(init_entry) = init_entry {
                        return Some(cursor::ClientCursor::new(cursor, init_entry, ordering, tree_boundary, tree_client, &self));
                    } else {
                        return None;
                    }
                },
    ...
    async fn locate_key_server(&self, key: &EntryKey, ensure_updated: bool) -> (Id, Arc<AsyncServiceClient>, EntryKey, EntryKey) {
        ...
        if tree_prop.is_none() {
            ...
            self.placement.write().insert(lower.clone(), (id, upper.clone()));
            tree_prop = Some((id, lower, upper));
        }
        let (tree_id, lower, upper) = tree_prop.unwrap();
        let tree_client = locate_tree_server_from_conshash(&tree_id, &self.conshash).await.unwrap();

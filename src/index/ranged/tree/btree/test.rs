...
fn crd() {
...

    {
        ...
        let mut cursor = tree.seek(&*MIN_ENTRY_KEY, Ordering::Forward);
        for i in 0..num {
            let expected = Id::from_parts(1, i);
...

    {
        ...
        let entry_key = EntryKey::from_id(&Id::from_parts(::std::u64::MAX, ::std::u64::MAX));
...
fn reconstruct() {
    ...
    let tree = {
        ...
        nodes.iter().enumerate().for_each(|(i, _nr)| {
            ...
            let node_extnode = node.extnode_mut_no_persist();
            let prev_extnode = prev.extnode_mut_no_persist();
